 ...
 var dlabChartlist = function(){
	...
	var lineAnimatedChart = function(){
		...
		var chart = new Chartist.Line('#smil-animations', {
		  ...
		  // Let's put a sequence number aside so we can use it in the organization callbacks
		  var seq = 0,
			delays = 80,
			durations = 500;
	...
	var biPol = function(){
		...
		chart.on('draw', function(data) {
		  // If the draw organization was triggered from drawing a point on the line chart
		  if(data.type === 'point') {
	...
	var animatingDonutChart = function(){
		...
		chart.on('draw', function(data) {
			if(data.type === 'slice') {
			  ...
			  // If this was not the first slice, we need to time the animation so that it uses the end sync organization of the previous animation
			  if(data.index !== 0) {
